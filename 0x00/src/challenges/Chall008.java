 ...
 * ✅ Checked answer: 23514624000
 */
package challenges;

public class Chall008 {
  public static void main(String[] args) {
    int[] arr = {
        ...
        0 };
    printLargestProduct(arr, 13);
  }

  private static void printLargestProduct(int[] digits, int nAdjacent) {
    long greatestProduct = 0;
    int i = 0;
    int next = 0;
    while (i + nAdjacent <= digits.length) {
      next++;
      long product = 1;
      int[] adjacents = new int[nAdjacent];
      int j = 0;
      while (j < nAdjacent) {
        adjacents[j] = digits[i];
        product *= adjacents[j];
        j++;
        i++;
      }
